...
for(i in 1:r){
  theta1 <- theta1+l1[r]*c1o[,r]%o%c2o[,r]%o%(c3o==-1)[,r]
  theta2 <- theta2+l2[r]*c1o[,r]%o%c2o[,r]%o%(c3o==1)[,r]
}
theta1 <- theta1+b1
theta2 <- theta2+b2
...
c3track <- list()
  ...
  #//initialization
 al=1;bl=3;av=1;bv=1;am=1;bm=1;sigmal=1;mub=0;sigmab=0.1;arho=1;brho=1;psi_1=1/3;psi0=1/3;psi1=1/3;as=1;bs=1;sigmamu=1;
...
lambda1 <- matrix(rgamma(d[3], al, 1/bl), d[3])

lambda2 <- matrix(rgamma(d[3], al, 1/bl), d[3])
  #//paramters
  ...
  for(it in 1:2000){
    ...
    #v1 <- li4[[2]]
    #v2 <- li4[[3]]
    li1 <- update_c1(c1,c2,c3,lambda1,lambda2,d,z,b1,b2)
    c1 <- li1[[1]]
    c2 <- li1[[2]]
    c3 <- li1[[3]]
    ...
    li3 <- new_feature(c1,c2,c3,lambda1,lambda2,d,z,b1,b2,gamma, m,rho,al,bl)
    c1 <- li3[[1]]
    c2 <- li3[[2]]
    c3 <- li3[[3]]
    lambda1 <- li3[[4]]
...
```{r}
pc2o = melt(t(c3o), varnames = c("Factor", "secondorder"), value.name = "Indicator")
pc2p = melt(t(c3), varnames = c("Factor", "secondorder"), value.name = "Indicator")
```
...
pc3p = melt(t(c3p), varnames = c("Factor", "thirdorder"), value.name = "Indicator")
```
